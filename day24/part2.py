...
def main(input_file):
	...
	for i in range(cycle):
		graph[(i,0,1)] = list()
		graph[(i,bounds["row"],bounds["col"]-1)] = list()		
		graph[(i,0,1)].append( ((i+1) % cycle,0,1) )
		graph[(i,bounds["row"],bounds["col"]-1)].append( ( (i+1) % cycle,bounds["row"],bounds["col"]-1 ) ) 
		if (1,1) in empty_space[(i+1) % cycle]:
			graph[(i,0,1)].append( ((i+1) % cycle,1,1) )
		if (bounds["row"]-1,bounds["col"]-1) in empty_space[ (i+1) % cycle]:
			graph[(i,bounds["row"],bounds["col"]-1)].append( ( (i+1) % cycle, bounds["row"]-1, bounds["col"]-1 ) )
	...
	start = (0,0,1)
	end_row = bounds["row"]-1
	end_col = bounds["col"]-1
	total_len += dijkstra(graph, start, end_row, end_col, cycle)
	
	start = ((total_len%(cycle-1)),bounds["row"],bounds["col"]-1)	
	end_row = 1
	end_col = 1
	total_len += dijkstra(graph, start, end_row, end_col, cycle)
	
	start = (total_len%(cycle-1),0,1)
	end_row = bounds["row"]-1
	end_col = bounds["col"]-1
	total_len += dijkstra(graph, start, end_row, end_col, cycle)
...
def dijkstra(graph, start, end_row, end_col, cycle):
